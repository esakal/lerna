...
var Package = function () {
  ...
  _createClass(Package, [{
    ...
    key: "getPublishDirectoryPackage",
    value: function getPublishDirectoryPackage(resolver) {
      if (typeof this._publishDirPackage === "undefined") {
        if (this.publishDirectoryLocation) {
          var publishDirLocation = this.publishDirectoryLocation;
          var publishDirConfigPath = _path2.default.join(publishDirLocation, "package.json");
          var publishDirJson = null;

          _npmlog2.default.verbose("package " + this.name + " is configured to publish from custom " + "directory " + publishDirLocation);

          if (!_FileSystemUtilities2.default.existsSync(publishDirConfigPath)) {
            if (resolver) {
              publishDirJson = resolver(null);
            }
          } else {
            publishDirJson = _readPkg2.default.sync(publishDirConfigPath, { normalize: false });
          }

          if (publishDirJson) {
            this._publishDirPackage = new Package(publishDirJson, publishDirLocation);
          } else {
            var message = "Package " + this.name + " is configured to publish from custom directory " + "which doesn't exists or missing 'package.json'";
    ...
    get: function get() {
      return this._package.config && this._package.config["publishDirectory"] ? this._package.config["publishDirectory"] : null;
    }
  }, {
    key: "publishDirectoryLocation",
    get: function get() {
      return this.publishDirectory ? _path2.default.join(this.location, this.publishDirectory) : null;
    }
  }, {
    key: "version",
    get: function get() {
